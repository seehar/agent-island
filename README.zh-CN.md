...
|**智能体**|**全部启用并安装** / **全部关闭并卸载** 两条批量动作；每个 Agent 的启用开关、集成状态与落点、审批闸门盾牌（`omp` / `pi`），以及每行可展开的**配置目录**编辑器（自动检测 / 选择文件夹 / 恢复默认）· **审批闸门**卡片：闸门问什么（写与命令 / 只问危险命令 / 始终允许）、应用未运行时怎么办、待批时自动展开（仅当决定只能在刘海上给 / 总是 / 从不）|
