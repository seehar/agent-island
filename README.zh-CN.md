...
|**额度**|你自己 New API 实例的账户余额与当前密钥额度 —— **可以配多个账号**，每个账号各填服务器地址、API 密钥，以及可选的访问令牌与用户 ID；从刘海顶部的信用卡按钮进入（同样不占分段位），只在打开这一页时取数。|
...
- 联网行为只有两处、且都是只读：Sparkle 查更新（本仓库 GitHub Pages 上的 appcast），以及你在 **设置 → 额度** 里填了服务器之后、打开那一页时对**你自己的** New API 实例发的两个 GET（`/api/usage/token/` 与 `/api/user/self`）——按账号各发一次，某个槽位没填凭据时那个端点根本不发。此外不向任何地方发送数据，也不在后台轮询。
