...
|**智能体**|**全部启用并安装** / **全部关闭并卸载** 两条批量动作；每个 Agent 的启用开关、集成状态与落点，以及每行可展开的**配置目录**编辑器（自动检测 / 选择文件夹 / 恢复默认）。启用 `omp` / `pi` 会同时装好它们的刘海审批扩展（没有第二个开关）· **工具调用保护**卡片：运行前询问什么（写与命令 / 只问危险命令 / 始终允许）、AgentIsland 未运行时怎么办、有待处理请求时自动展开（仅当决定只能在刘海上给 / 总是 / 从不）|
