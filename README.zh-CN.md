...
- **统计** —— 头部再多一个按钮：token、会话数与工具调用次数的用量页，按 Agent 拆分，分今天 / 本周 / 本月 / 全部。
...
**用量统计。** 头部图表按钮打开统计页：token 总量（含输入 / 输出 / 缓存读 / 缓存写与命中率）、会话数与工具调用次数，按 **今天 / 本周 / 本月 / 全部** 分档，并按 Agent 拆分，另带趋势柱图与工具榜。数字来自对 Agent 自身会话记录的索引，已结束的会话也计入。
...
统计页读的是与对话页同一批记录，只把聚合计数留在应用自己的存储里。总量包含缓存 token（命中率 = 缓存读 /（输入 + 缓存读 + 缓存写））；会话数不计子代理。首次打开时在后台索引，页面会显示最近一次索引时间。
...
|页面|内容|
|---|---|
|**通用**|语言、屏幕、胶囊高度、胶囊宽度、内容字号、通知音效 · 登录时启动、辅助功能状态|
|**智能体**|每个 Agent 的启用开关、集成状态与落点、审批闸门盾牌（`omp` / `pi`）与它问什么（写与命令 / 只问危险命令）、Claude Code 配置目录|
|**行为**|悬停展开（从不 / 快 / 标准 / 慢）、空闲胶囊（一直显示 / 有活动时 / 保留 3 秒）、待批时自动展开（仅当决定只能在刘海上给 / 总是 / 从不）、完成提示（10 秒 / 30 秒 / 1 分钟 / 一直）、面板尺寸（紧凑 / 标准 / 宽） · 已结束会话保留、行信息密度、单击动作（无 / 打开对话 / 聚焦终端）、刷新频率 · 提示音范围（仅就绪 / 就绪与审批）|
...
**应用不在跑时**，集成不会挡你的路：Claude Code 回落到自己的提示，OpenCode 把审批交还它的 TUI，`omp`/`pi` 的闸门按自身的降级档处理（默认 `notify-only`：放行并在终端里留可见提示；`strict` 一律拒绝；`read-only-allow` 拒绝写入与执行）。危险命令在任何档位下都拒绝。而应用在跑、但你超时没答时，闸门会拒绝 —— 没人应答不等于同意。
