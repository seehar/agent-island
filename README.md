...
|**Quota**|Account balance and current key balance from your own New API instance — server URL, API key, and the optional access token / user ID. Reached from the card button in the panel header (it takes no tab slot); it fetches only while the page is open.|
...
- Two network calls exist, both read-only: the Sparkle update check against this repository's appcast on GitHub Pages, and — after you fill in a server under **Settings → Quota** — two GETs to *your own* New API instance while that page is open (`/api/usage/token/` and `/api/user/self`). Nothing else is sent anywhere, and nothing is polled in the background.
