...
/// 用量数字的短格式。
///
/// **缩写值一律保留 2 位小数**（一位小数会让 12,488 显示成 1.2万，误差 3.9%；两位时
/// 1.25万 的误差在 0.5% 以内）；不足一个量级的原数按整数显示，不给整数补 `.00`
/// （token 数本身是整数）。
///
/// 边长的那几档（`1234.57万` / `695.39亿`）靠**版面**承接，不靠砍小数位：曲线图的
/// y 轴刻度栏按最长刻度文案实测宽度（`UsageStatsMetrics.chartYAxisWidth(forLabelWidths:)`），
/// 总览卡的大数字也有「最长取值 + 两个小计仍排得下」的用例守着。
///
/// 中文界面按「万 / 亿」（12,488 → 1.25万；69,538,549,758 → 695.39亿），其余语言沿用
/// K / M / B 公制词头（词头各语言通用；B 那一档不能省——只到 M 的话 32 亿 token 会写成
/// `3239M`，比中文的 `32.39亿` 还长）。缩写值不带千分位（`1234.57万` 而不是
/// `1,234.57万`：缩写值本身是近似值，位数越短越好），原数那一档仍按语言加千分位。
nonisolated enum UsageTokenFormat {
    ...
    /// 缩写值的写法：一律 2 位小数、不带千分位。
    ///
    /// 不传 locale 给 `String(format:)`：C 的 `%.2f` 不加分组分隔符，`1234.57万` 因此比
    /// `1,234.57万` 短两个字符（原数那一档 `9,999` 仍按语言加千分位）。
